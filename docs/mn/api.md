...
- `getallheaders()` байвал -> PSR-7 headers (`$this->headers`) болон serverParams (`HTTP_*` формат) дотор нэгтгэнэ
- `$_COOKIE` -> cookies
- `$_FILES` -> uploadedFiles (normalize хийж)
- `php://input` / `$_POST` -> parsedBody

`getallheaders()` ашиглан бүх HTTP header-үүдийг `setHeader()` дуудан PSR-7 `$this->headers` массивд бүртгэдэг тул `getHeaderLine()`, `getHeader()`, `hasHeader()` зэрэг PSR-7 функцүүд бүх header-ийг зөв уншина.
